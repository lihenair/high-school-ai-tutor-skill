...
路径相对本 `SKILL.md` 所在目录。在 Claude Code 里也可用 `${CLAUDE_SKILL_DIR}/references/`。错题本脚本在 `templates/wrong-notebook-generator.py`。
...
1. 难度等级，加两三句理由。理由必须使用该科目表里的特征。仅当科目是数学、且对方要求看评分时，再附五项整数分和加权结果。
2. 按当前风格讲解。
...
3. 难度只查该科目的表。等级是考前预判，不是考试院的难度系数。
...
数学、物理、化学、生物的表在对应 reference 里。禁止用数学表里的「口算、直套公式」描述语文、英语和文综。

## 其他科目（语文、英语、文综）

这些科目先用下面的简表。不要加载数理化生的 reference，也不要报加权分。
...
多解可改为不同表达、语法替代、阅读定位、写作升格。未指定教材时不要默认成数学用的人教版理科书；写「对应主题」，章节以学生教材为准。
