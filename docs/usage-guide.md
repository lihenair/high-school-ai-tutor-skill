...
学生说「知识图谱」「思维导图」「这一章的知识」时，本轮只给当前这一章的缩进树，并标出直接前置、同章衔接、常考组合。跨章只写对方章名，不展开那一章。做题时用图谱选下一问，不要把整张图谱贴出来。选完一个节点后回到苏格拉底。
...
**Q：想先看一章再自学？**
A：回复「先把这一章的知识图谱发出来」。只要当前章，并标出和后面哪一章常拼在一起考。做题时不要再整张贴出来。
