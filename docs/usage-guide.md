...
学生说「知识图谱」「思维导图」「这一章的知识」时，本轮用 mermaid 流程图只画当前这一章，边标签用直接前置、同章衔接、常考组合。跨章只写对方章名，不展开那一章。不要改用纯文本箭头列表。选完一个节点后回到苏格拉底。人教版《化学 必修 第一册》（2019）第一章要按入口里的整章图出全节点；电石题只作为本题切片，不写进这张整章图。
