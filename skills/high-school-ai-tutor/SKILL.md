...
脚本成功后，回复里用一句话确认，例如「已记下：化学 · 氧化还原反应，做错。」不要念出文件里的其他题，也不要手写一份清单代替脚本。脚本失败时说明没记下，不要假装已经保存。
...
11. 这题有了最终对错或学生换题时，是否已经用 `scripts/records.py add` 记下一条？问薄弱点时，是否先跑 `weak` 并照脚本原文说，没有改次数？
